<div align="center">

# 🎬 沫兮万能解析

**PHP 智能线路切换系统 · NoAd 去广告 · MD5 深度分析 · 单文件后台 v5.2.0**

> 多线路并发请求 · 智能选优 · M3U8 直链检测 · 全平台视频解析 · 5 大模块可视化后台 · M3U8 广告片段智能过滤 · MD5 深度分析去广告 · 智能自动保存 · 代理/速率限制反封禁

[![PHP](https://img.shields.io/badge/PHP-7.4%2B-777BB4?logo=php&logoColor=white)](https://www.php.net/)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
[![Status](https://img.shields.io/badge/Status-✅%20Active-brightgreen.svg)]()
[![安装指南](https://img.shields.io/badge/📖-新手安装指南-ff69b4.svg)](INSTALL.md)
[![快速上手](https://img.shields.io/badge/🚀-5%20%E5%88%86%E9%92%9F%E9%83%A8%E7%BD%B2-orange.svg)](INSTALL.md#3%EF%B8%8F%E2%83%A3%E6%9C%AC%E8%BF%90%E8%A1%8C%E7%A8%8B)

> 💡 **新手看这里**：第一次使用？👉 **[点击查看完整安装指南](INSTALL.md)**（中文，从零开始手把手教学）

</div>

---

## 🏗️ 项目整体架构图

```
                        ┌─────────────────────┐
                        │   👤 外部用户请求    │
                        │  (浏览器/APP/脚本)  │
                        └───────┬─────────────┘
                                │
        ┌───────────────────────┼───────────────────────────┐
        ▼                       ▼                           ▼
┌─────────────────┐   ┌─────────────────┐       ┌─────────────────┐
│  🎯 index.php   │   │  🔐 admin.php   │       │  🔄 noad_proxy.php│
│  (解析接口入口)  │   │  (后台管理入口)  │       │  (代理/清洗入口)  │
└──────┬──────────┘   └──────┬──────────┘       └──────┬──────────┘
       │                      │                         │
       ├──────────────────────┼─────────────────────────┤
       ▼                      ▼                         ▼
┌─────────────────┐   ┌─────────────────┐       ┌─────────────────┐
│  🧹 noad_md5.php│   │  ⚡ q.php       │       │  🔍 deep_analysis.php│
│ (MD5去广告接口)  │   │ (去广告专用接口) │       │  (深度分析工具)   │
└──────┬──────────┘   └──────┬──────────┘       └──────┬──────────┘
       │                      │                         │
       └──────────────────────┼─────────────────────────┘
                              ▼
┌───────────────────────────────────────────────────────────────────────┐
│                      🏗️  核心层 (core/)                                  │
├───────────────────────────────────────────────────────────────────────┤
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ │
│  │🧠 Strategy   │ │🌐 Requester  │ │💾 SmartCache │ │⚡ SmartConcurrency │
│  │ 智能选优     │ │ 并发请求     │ │ 文件缓存     │ │ 双层并发     │ │
│  └──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └──────┬───────┘ │
│         │                 │                 │                 │         │
│         ▼                 ▼                 ▼                 ▼         │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ │
│  │✂️ NoAdParser │ │🗄️ Database   │ │🔑 MD5FingerprintDB │ │📊 PatternFeatureDB │
│  │ 广告过滤     │ │ SQLite 持久   │ │ 指纹库自学习  │ │ 模式特征识别 │ │
│  └──────┬───────┘ └──────────────┘ └──────────────┘ └──────────────┘ │
│         │                                                              │
│         └──────────────────────┬───────────────────────────────────────┘
│                                ▼                                        │
│         ┌─────────────────────────────┐                               │
│         │ 🎬 M3U8Handler (handlers/)   │                               │
│         │ 直链检测 + 快速响应           │                               │
│         └──────────────┬──────────────┘                               │
│                        ▼                                               │
│         ┌─────────────────────────────┐                               │
│         │ 🧩 algorithms/ 自定义算法     │                               │
│         │ (如意/MD5/模式特征/序列跳跃)  │                               │
│         └──────────────┬──────────────┘                               │
│                        ▼                                               │
│         ┌─────────────────────────────┐                               │
│         │ 🛠️ tools/ 工具模块            │                               │
│         │ (广告清理/批量解析/特征提取)  │                               │
│         └──────────────┬──────────────┘                               │
│                        ▼                                               │
│         ┌─────────────────────────────┐                               │
│         │ 📂 config/ 配置层             │                               │
│         │ (api/platform/switch/        │                               │
│         │  admin/noad/global_rules)    │                               │
│         └──────────────┬──────────────┘                               │
└────────────────────────┼───────────────────────────────────────────────┘
                         ▼
         ┌───────────────────────────────────┐
         │  📄 解析后 JSON 响应                │
         │  /  📼 清洗后的 M3U8                │
         │  /  💾 缓存数据 / 指纹库             │
         └───────────────────────────────────┘
```

## 📖 项目简介

**沫兮万能解析** 是一个基于 PHP cURL 的多线路视频解析系统，采用**并发请求 + 智能选择策略**，自动从多条 API 线路中选择响应最快的有效结果，同时支持 M3U8 直链快速通道。

### ✨ 核心特性

| 功能 | 说明 |
|------|------|
| ⚡ **并发请求** | 同时向所有 API 线路发起请求，优先返回最快响应 |
| 🎯 **智能选优** | 三级选择算法：平台优先级 → 总接口最快 → 全局最快 |
| 📺 **M3U8 直链** | 自动识别 m3u8 链接，无需走解析流程 |
| ✂️ **NoAd 去广告** | 自动识别 M3U8 中的广告片段并移除，可配置解析源 |
| 🧩 **自定义算法** | 可扩展的算法注册表，支持 m3u8/url 多作用域清洗 |
| 🔧 **动态配置** | 通过配置文件增减 API，无需修改代码 |
| 🌐 **全平台适配** | 支持 17+ 视频平台智能匹配 |
| 🎛️ **多开关控制** | 支持自定义接口 / 统一格式 / 总接口 等开关 |
| 💾 **智能缓存** | 内置文件缓存系统，减少重复请求 |
| 🛡️ **代理/速率限制** | 支持多代理轮换 + 请求速率限制，避免 IP 被封禁 |
| 🔐 **可视化后台** | 单文件 `admin.php`，7 主标签 + 子标签分层，密码登录保护 |
| 🎯 **MD5广告过滤** | 基于片段MD5特征自动识别广告，智能学习 |
| 📋 **批量解析** | 批量处理多个视频源，自动生成报告 |
| 🔄 **规则自动更新** | 在线更新广告规则库，支持备份和回滚 |
| 💾 **智能自动保存** | 点击空白区域自动保存表单，localStorage 缓存可还原 |

---

## 🚀 快速开始

> 💡 **新手建议**：这里是最简操作步骤。如果看不懂或遇到问题，请阅读
> **[📖 完整新手安装指南](INSTALL.md)**（包含 Windows/macOS/Linux/宝塔面板/虚拟主机
> 全平台图文教程 + 常见问题 FAQ + 配置详解）。

### 环境要求

- **PHP** >= 7.4（兼容 7.4 ~ 8.5，推荐 8.0+）
- **cURL** 扩展（`curl_multi` 支持，核心依赖，必须启用）
- **pdo_sqlite** 扩展（可选，用于 NoAd 数据统计；未启用时核心解析照常工作）
- Web 服务器（Nginx / Apache 均可）

> 🔍 **如何确认环境？** 新建 `phpinfo.php` 写入 `<?php phpinfo(); ?>`，访问后搜索 `curl`，看到 `enabled` 即为通过。详细步骤见 **[INSTALL.md](INSTALL.md#1%EF%B8%8F%E2%83%A3%E7%8E%AF%E5%A2%83%E8%A6%81%E6%B1%82)**

### 部署步骤（3 步）

```bash
# 1. 下载代码
git clone https://github.com/ssmhdssmhd/MX-jx.git
cd MX-jx

# 2. 上传到 PHP 网站根目录（如宝塔的 /www/wwwroot/jx.example.com/）
#    或本地测试：php -S 0.0.0.0:8080

# 3. 浏览器测试（替换为你的域名或 localhost）
# http://你的域名/index.php?url=https://v.qq.com/x/cover/mzc00200mp8vo9l/r00421n6n7k.html
```

> 🪟 **Windows 用户** 推荐使用 **phpStudy**（小皮面板），详见 **[INSTALL.md · Windows 教程](INSTALL.md#%EF%B8%8F-windows-%E7%94%A8%E6%88%B7phpstudy-%E6%96%B9%E6%A1%88%E6%8E%A8%E8%8D%90-)**
>
> 🐧 **Linux 服务器** 推荐使用 **宝塔面板**，详见 **[INSTALL.md · 宝塔面板教程](INSTALL.md#%EF%B8%8F-%E6%96%B9%E6%A1%88%E4%B8%80%E5%AE%9D%E5%A1%94%E9%9D%A2%E6%9D%BF%E5%9B%BD%E5%86%85%E6%9C%8D%E5%8A%A1%E5%99%A8%E6%8E%A8%E8%8D%90)**

### 调用方式

```
GET  /index.php?url=<视频播放页地址>
POST /index.php  (body: url=<视频播放页地址>)
```

**示例（复制到浏览器测试）**：

```
https://你的域名/index.php?url=https://v.qq.com/x/cover/mzc00200mp8vo9l/r00421n6n7k.html
https://你的域名/index.php?url=https://www.iqiyi.com/v_xxx.html
https://你的域名/index.php?url=https://www.douyin.com/video/xxx
```

> 📖 **更多使用场景**（配合播放器使用 / 作为 API 接入 / 自定义接口管理）见 **[INSTALL.md · 使用方法](INSTALL.md#5%EF%B8%8F%E2%83%A3%E4%BD%BF%E7%94%A8%E6%96%B9%E6%B3%95)**

---

## 📁 项目结构

```
MX-jx/
├── index.php                  # 🎯 主入口文件（解析接口，对外 API）
├── noad_proxy.php             # 🔄 NoAd 代理（M3U8 清洗 / TS 跨域 / 自定义算法 API）
├── noad_md5.php               # 🧹 NoAd MD5 去插播专用接口（MD5 深度分析 + 自动去广告）
├── q.php                      # ⚡ 去广告专用接口（简洁 JSON 格式，第三方调用友好）
├── deep_analysis.php          # 🔍 深度分析工具页（M3U8 广告深度分析可视化）
├── admin.php                  # 🔐 后台管理入口（单文件，5 大模块 + 子标签分层）
├── disclaimer.php             # 📝 免责声明类（自动加载，无需手动管理）
├── ZJK.txt                    # ⚙️ 自定义接口配置（纯文本，每行一条）
├── cookies.txt                # 🍪 Cookie 缓存文件
├── tests_smoke.php            # 🧪 冒烟测试脚本（16 项快速验证）
├── test_noad_full.php         # 🧪 NoAd 完整功能测试脚本
├── update.sh                  # 🔄 沫兮客户端自动更新脚本
├── README.md                  # 📖 项目说明文档（本文）
├── INSTALL.md                 # 🚀 新手安装指南（从零开始的完整教程）
├── CHANGELOG.md               # 📋 版本更新日志
├── .htaccess                  # Apache 伪静态 + 安全防护配置
│
├── config/                    # 📂 配置层（所有可配置项集中在这里）
│   ├── api.php                # API 线路配置（多条主线路 + 备用线路）
│   ├── platform.php           # 平台规则配置（视频平台 → 优先解析接口的映射）
│   ├── switch.php             # 系统开关配置（总接口 / 缓存 / M3U8 直链 / NoAd / SQLite / 去插播）
│   ├── admin.php              # 后台配置（密码哈希 / 端口限制 / IP 白名单 / 会话有效期）
│   ├── noad.php               # NoAd 去广告配置（解析源 / 关键词 / 缓存 / 代理 / 算法 / MD5）
│   ├── global_rules.php       # 🌍 全局规则配置（通用广告规则 / 过滤规则）
│   └── 配置.md                # 📖 配置详细说明文档
│
├── core/                      # 📂 核心层（核心业务逻辑，通常不需要修改）
│   ├── strategy.php           # 🧠 Strategy - 智能选择策略（平台匹配 + 选优算法）
│   ├── requester.php          # 🌐 Requester - 并发请求引擎（curl_multi 并发 + 结果聚合）
│   ├── cache.php              # 💾 SmartCache - 文件缓存（过期自动清理）
│   ├── Database.php           # 🗄️ SQLite 持久层（解析源 / 规则 / 统计 / 日志）
│   ├── NoAdParser.php         # ✂️ M3U8 去广告 + 资源站清理 + 自定义算法入口
│   ├── MD5FingerprintDB.php   # 🔑 MD5 指纹库 - 广告特征码存储与匹配（自学习算法）
│   ├── PatternFeatureDB.php   # 📊 模式特征库 - 广告模式特征提取与识别
│   └── SmartConcurrency.php   # ⚡ 智能并发 - 多进程 + curl_multi 双层并发架构
│
├── handlers/                  # 📂 处理器层（特殊格式的快速处理）
│   ├── M3U8Handler.php        # 🎬 M3U8 直链检测与快速响应
│   └── 规则.md                # 📖 M3U8 处理器规则说明
│
├── algorithms/                # 📂 自定义算法扩展层（可插拔，12 个文件）
│   ├── AbstractAlgorithm.php  # 算法基类（priority / scope / matchPatterns 统一接口）
│   ├── AlgorithmRegistry.php  # 算法注册表（自动扫描 / 排序 / 按作用域应用）
│   ├── example_01_url_tracker_strip.php  # 示例：URL 跟踪参数清洗
│   ├── example_02_ad_keyword_filter.php  # 示例：广告关键词过滤
│   ├── example_03_domain_rewrite.php     # 示例：广告域名重写
│   ├── example_04_m3u8_segment_cleaner.php # 示例：M3U8 片段级清洗
│   ├── example_05_protocol_normalize.php # 示例：协议 / 路径规范化
│   ├── custom_template.php    # 用户自定义算法模板（复制后改写即可）
│   ├── ruyi_pattern_cleaner.php    # 🎯 如意算法：基于时长分布的智能广告识别
│   ├── md5_pattern_cleaner.php     # 🔑 MD5 指纹算法：基于片段 MD5 特征的广告匹配
│   ├── pattern_feature_cleaner.php # 📊 模式特征算法：基于文件名/大小模式的广告识别
│   └── sequence_jump_cleaner.php   # ⏭️ 序列跳跃算法：基于不连续点的插播识别
│
├── tools/                     # 📂 工具模块（独立工具，可单独调用）
│   ├── core/
│   │   ├── AbstractTool.php   # 工具基类
│   │   └── ToolManager.php    # 工具管理器（注册 / 调用 / 生命周期管理）
│   ├── ad_cleaner/
│   │   └── AdCleanerTool.php  # 🧹 广告清理工具
│   ├── batch_parse/
│   │   └── BatchParseTool.php # 📋 批量解析工具
│   ├── common_ad_rules/
│   │   └── CommonAdRuleTool.php # 📜 通用广告规则工具
│   ├── feature_extractor/
│   │   └── FeatureExtractorTool.php # 🔬 特征提取工具
│   ├── md5_ad_filter/
│   │   └── MD5AdFilterTool.php # 🔑 MD5 广告过滤工具
│   ├── rule_update/
│   │   └── RuleUpdateTool.php # 🔄 规则自动更新工具
│   ├── bootstrap.php          # 工具模块启动引导
│   └── handler.php            # 工具请求处理器
│
├── html/                      # 📂 前端页面（纯静态，可独立使用）
│   ├── index.html             # 🏠 首页 / 演示页（输入视频链接一键解析播放）
│   ├── player.html            # ▶️ M3U8 播放器（基于 HLS.js，点播 + 直播）
│   ├── api-test.html          # 🧪 API 调试页（GET/POST 测试 + JSON 响应可视化）
│   └── m3u8-test.html         # 📺 M3U8 测试页（内置公开测试流）
│
├── docs/                      # 📂 文档目录（开发 / 运维参考）
│   ├── 配置说明.md            # 配置文件详细说明（各字段含义 + 取值范围）
│   ├── 核心说明.md            # 核心模块设计说明（架构 + 流程 + 扩展点）
│   ├── M3U8处理器规则.md      # M3U8 处理器规则说明（匹配条件 + 响应格式）
│   └── 源码运行调用视图.md      # 📊 源码调用流程图：解析流程 + 去广告/去插播流程
│
├── dist/                      # 📂 发布目录（生产部署副本）
│   ├── algorithms/            # 算法文件（发布副本）
│   ├── cache/                 # 缓存目录（含 noad.db）
│   ├── config/                # 配置文件（发布副本）
│   ├── core/                  # 核心文件（发布副本）
│   ├── html/                  # 前端页面（发布副本）
│   ├── tools/                 # 工具模块（发布副本）
│   ├── admin.php              # 后台入口（发布副本）
│   ├── index.php              # 主入口（发布副本）
│   ├── noad_proxy.php         # 代理入口（发布副本）
│   └── q.php                  # 去广告接口（发布副本）
│
├── test_video/                # 📂 测试视频（用于本地测试）
│   ├── test.m3u8              # 测试用 M3U8 播放列表
│   ├── ad_001.ts ~ ad_003.ts  # 广告片段测试文件
│   └── content_001.ts ~ content_005.ts # 正片片段测试文件
│
├── cache/                     # 📂 缓存目录（运行时自动生成，默认不提交）
│   ├── batch_reports/         # 批量解析报告
│   ├── deep_analysis/         # 深度分析结果
│   ├── noad_md5/              # NoAd MD5 缓存（JSON + M3U8）
│   └── .gitkeep               # 空目录占位符
│
└── backup/                    # 📂 后台「一键备份」输出目录（默认不提交）
    └── .gitkeep               # 空目录占位符
```

### 📊 项目文件统计（v5.1.2）

| 类型 | 数量 | 说明 |
|------|------|------|
| **入口文件** | 7 个 | `index.php` / `admin.php` / `noad_proxy.php` / `noad_md5.php` / `q.php` / `deep_analysis.php` / `disclaimer.php` |
| **后台文件** | 1 个 | `admin.php`（单文件整合，CSS + 模板 + 逻辑全部内嵌，5 大模块） |
| **配置文件** | 6 个 | `config/` 目录下 6 个 PHP 配置文件 + 1 个说明文档 |
| **核心模块** | 8 个 | `core/` 目录下 8 个核心类 |
| **处理器** | 1 个 | `handlers/M3U8Handler.php` |
| **自定义算法** | 12 个 | `algorithms/` 目录下（基类 + 注册表 + 5 示例 + 4 实算法 + 1 模板） |
| **工具模块** | 8 个 | `tools/` 目录下（核心 + 6 工具 + 引导 + 处理器） |
| **前端页面** | 4 个 | `html/` 目录下 4 个纯静态 HTML |
| **文档** | 5 个 | README / INSTALL / CHANGELOG / docs/ 3 个说明 |
| **测试文件** | 2 个 | `tests_smoke.php` / `test_noad_full.php` |
| **总文件数** | ~50+ 个 | |

### 核心类 / 文件说明

| 文件 | 类型 | 职责 |
|------|------|------|
| `Strategy` | [core/strategy.php](core/strategy.php) | 策略决策引擎：平台匹配、API 聚合、响应选优 |
| `Requester` | [core/requester.php](core/requester.php) | 请求执行引擎：`curl_multi` 并发请求、结果解析、格式统一 |
| `SmartCache` | [core/cache.php](core/cache.php) | 缓存管理器：文件系统缓存、过期自动清理 |
| `M3U8Handler` | [handlers/M3U8Handler.php](handlers/M3U8Handler.php) | M3U8 处理器：直链检测、快速响应 |
| `NoAdParser` | [core/NoAdParser.php](core/NoAdParser.php) | M3U8 广告片段识别过滤 / 多源并发匹配 / 资源站清理 / 自定义算法入口 |
| `Database` | [core/Database.php](core/Database.php) | SQLite 轻量持久层：解析源 / 广告规则 / 访问统计 / 日志 |
| `MD5FingerprintDB` | [core/MD5FingerprintDB.php](core/MD5FingerprintDB.php) | 🔑 MD5 指纹库：广告特征码存储、匹配、自学习（越用越准） |
| `PatternFeatureDB` | [core/PatternFeatureDB.php](core/PatternFeatureDB.php) | 📊 模式特征库：广告文件名模式、大小分布特征提取与识别 |
| `SmartConcurrency` | [core/SmartConcurrency.php](core/SmartConcurrency.php) | ⚡ 智能并发：多进程（pcntl）+ curl_multi 双层并发架构 |
| `ToolManager` | [tools/core/ToolManager.php](tools/core/ToolManager.php) | 🛠️ 工具管理器：工具注册、调用、生命周期统一管理 |

### 后台管理（单文件 admin.php，5 大模块 + 子标签分层）

| 主模块 | 功能说明 | 子标签 |
|--------|----------|--------|
| 📊 **仪表盘** | PHP 版本 / cURL 状态 / API 数量 / 环境检查 / 操作日志 / 快速开始 | - |
| 🔧 **解析配置** | 解析接口管理 / 自定义解析 / 平台规则 / API 设置 / 调用日志 / 解析日志 / 接口测试 | 解析接口管理 / 自定义解析 / 平台规则 / API 设置 / 调用日志 / 解析日志 / 接口测试 |
| 🚫 **去广告配置** | MD5 深度分析 / 总览 / 资源站设置 / 广告关键词规则 / 解析源管理 | MD5 深度分析 / 总览 / 资源站设置 / 广告关键词规则 / 解析源管理 |
| 🧩 **算法** | 算法列表 / 如意算法 / MD5指纹算法 / 万能规则2 / M3U8解析测试 | 算法列表 / 如意算法 / MD5指纹算法 / 万能规则2 / M3U8解析测试 |
| ⚙️ **系统设置** | 配置备份 / 修改密码 / 后台设置 / 缓存管理 / 工具管理 | 配置备份 / 修改密码 / 后台设置 / 缓存管理 / 工具管理 |

> 💡 **智能自动保存**：在后台点击空白区域时，系统会自动保存当前表单状态到浏览器 localStorage，并显示「✓ 已自动保存当前设置」提示。可通过「↩ 还原上次自动保存」按钮回滚到之前状态，避免误操作损失配置。

### 🔒 后台安全特性

- **密码加密存储**：管理员密码以 MD5 哈希形式保存在配置文件中
- **失败锁定**：连续登录失败超过 5 次自动锁定 300 秒
- **端口限制**：可配置仅允许特定端口访问后台
- **IP 白名单**：可配置允许访问后台的 IP 列表
- **会话超时**：登录会话自动过期（默认 2 小时）
- **操作日志**：记录管理员每一次修改操作

### 🛡️ 代理与反封禁（v4.3 新增）

在 `config/noad.php` 中可配置：

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `enable_proxy` | `false` | 是否启用 HTTP 代理池 |
| `proxies` | `[]` | 代理列表，支持 `ip:port` 格式，请求时自动轮换 |
| `rate_limit_min_interval_ms` | `350` | 同一域名请求最小间隔（毫秒） |
| `rate_limit_jitter_ms` | `150` | 随机抖动范围（毫秒），避免固定频率被识别 |
| `rate_limit_max_concurrent` | `3` | 同一域名最大并发请求数 |
| `user_agents` | `[...]` | 随机 User-Agent 池，每次请求随机选择 |

> 💡 如果你的解析接口对 IP 有频率限制或容易封禁，启用代理池 + 设置合理的速率限制可以有效降低被拦截的概率。

---

### 请求处理流程

```
                     HTTP 请求
                        │
                        ▼
          ┌──────────────────────────┐
          │   index.php  参数校验      │
          │   (URL 格式 + 非空校验 +   │
          │    SSRF 协议校验)          │
          └─────────────┬──────────────┘
                        │
              ┌─────────┴────────────┐
              ▼                      ▼
      是 M3U8 链接？            否 → 进入解析流程
              │                      │
              ▼                      ▼
     直接返回 JSON        ┌─────────────────────────┐
                          │ 模式判断 (mode 参数)       │
                          │  auto / noad / legacy     │
                          └──────────┬────────────────┘
                                     │
                    ┌────────────────┴───────────────┐
                    ▼                                ▼
        ┌───────────────────────┐        ┌───────────────────────┐
        │ NoAd 去广告模式         │        │ 旧版并发解析模式        │
        │ (noad + 自定义算法)     │        │ (Strategy + Requester)│
        └──────────┬─────────────┘        └──────────┬─────────────┘
                   ▼                                ▼
        ┌─────────────────────────────────────────────────────────┐
        │                    Strategy 决策引擎                      │
        │  · 匹配平台规则 · 聚合所有 API · 选最优线路                │
        └───────────────────────────┬─────────────────────────────┘
                                    ▼
                    ┌───────────────────────────┐
                    │   Requester 请求引擎       │
                    │   · curl_multi 并发请求    │
                    │   · 收集有效响应           │
                    │   · 代理池轮换             │
                    │   · 速率限制 + UA 随机     │
                    │   · Strategy 选最优        │
                    │   · 统一响应格式           │
                    └─────────────┬─────────────┘
                                  ▼
                          返回 JSON 响应
```

### 智能选优算法

```
                     所有并发响应
                          │
          ┌───────────────┴────────────────┐
          ▼                                ▼
   匹配平台规则？                      未匹配平台
     ├─► 返回 Priority API 响应         │
     └─► 失败则继续                      │
                                       ▼
                           过滤 HTTP 200 + 非空响应
                                       │
                          ┌────────────┴────────────┐
                          ▼                         ▼
                   有总接口响应？               无总接口
                     ├─► 选其中响应最快的           │
                     └─► 无则继续                    │
                                               选全局最快响应
```

---

## ⚙️ 配置说明

### API 线路配置 — [config/api.php](config/api.php)

```php
return [
    '接口名称' => '接口URL|超时秒数',
    // 示例:
    'MyJson1-VIP' => 'https://jx.jsonplayer.com/player/?url=|5',
];
```

- 系统内置 **多条主线路** + **备用线路**
- 格式：`接口URL|超时时间`（秒）
- 可自由增减，无需修改核心代码
- 可通过后台可视化编辑

### 平台规则配置 — [config/platform.php](config/platform.php)

```php
return [
    '平台名称' => '域名标识|优先使用的API名',
    // 示例:
    'qq'     => 'v.qq.com|MyJson1-VIP',
    'iqiyi'  => 'iqiyi.com|MyJson2-VIP',
];
```

已支持平台：腾讯视频、爱奇艺、抖音、芒果TV、优酷、PPTV、乐视、搜狐、1905电影、Bilibili、西瓜视频、Acfun 等。

### 系统开关 — [config/switch.php](config/switch.php)

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `enable_global_api` | `true` | 启用"总接口"模式，前 N 条 API 兼容全平台 |
| `global_api_count` | `6` | 作为总接口的 API 数量（前 N 条） |
| `global_api_timeout` | `8` | 总接口请求超时（秒） |
| `enable_zjk_apis` | `true` | 是否加载 `ZJK.txt` 自定义接口 |
| `enable_m3u8_direct` | `true` | M3U8 直链直接输出（不走解析） |
| `enable_unified_display` | `true` | msg 与 url 字段统一显示 |
| `noad_enabled` | `true` | NoAd 去广告系统总开关 |
| `sqlite_enabled` | `true` | SQLite 数据库开关（指纹库持久化） |
| `ad_remove_enabled` | `true` | 去插播广告总开关 |
| `video_parse_enabled` | `true` | 视频解析接口开关（`/?url=`） |
| `ad_remove_url_enabled` | `true` | 去广告链接接口开关（`/q.php?url=`） |
| `parse_integrate_noad` | `true` | 解析结果集成去广告链接数组 |
| `debug_mode_enabled` | `false` | 调试模式开关 |

### NoAd 去广告配置 — [config/noad.php](config/noad.php)

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `noad_enabled` | `true` | 是否启用 NoAd 去广告系统 |
| `ad_keywords` | `[...]` | 广告关键词列表（中英文均可） |
| `cache_ttl_seconds` | `1800` | 缓存有效期（秒，默认 30 分钟） |
| `request_timeout` | `10` | 请求超时时间（秒） |
| `max_concurrent_sources` | `3` | 最大并发解析源数 |
| `enable_proxy` | `false` | 是否启用 HTTP 代理池 |
| `proxies` | `[]` | 代理列表（ip:port 格式） |
| `rate_limit_min_interval_ms` | `350` | 同一域名最小请求间隔 |
| `user_agents` | `[...]` | User-Agent 随机池 |
| `enable_custom_algorithms` | `true` | 是否启用自定义算法扩展 |
| `custom_algorithms_scope` | `'m3u8'` | 自定义算法作用域（m3u8/url/all） |

### 自定义接口 — [ZJK.txt](ZJK.txt)

```
# 每行一个接口，格式: URL|超时秒数
# 注释行以 # 开头
https://jx.example.com/player/?url=|6
https://jx2.example.com/?url=|8
```

---

## 📤 响应格式

### 成功响应（JSON）

```json
{
    "code": 200,
    "msg": "https://player.example.com/video.m3u8",
    "url": "https://player.example.com/video.m3u8",
    "type": "m3u8",
    "MX": "检测到M3U8，直接输出",
    "strategy": "M3U8-Direct-Output",
    "developer": "MX-射手沫蝴蝶",
    "contact": "QQ: 2094332348",
    "timestamp": 1718768000,
    "switch_status": {
        "global_api_enabled": true,
        "global_api_count": 6,
        "zjk_apis_enabled": true,
        "zjk_file_exists": true,
        "unified_display_enabled": true
    }
}
```

### 策略标记说明

| strategy 值 | 说明 |
|-------------|------|
| `XXX-Priority[platform]` | 平台优先级 API 命中 |
| `XXX-Global-Fastest` | 总接口最快响应 |
| `XXX-Fastest` | 普通接口最快响应 |
| `XXX-Sequential` | 并发失败后顺序重试成功 |
| `M3U8-Direct-Output` | M3U8 直链直接输出 |

### 错误响应

```json
{
    "code": 400,
    "msg": "URL参数不能为空",
    "developer": "MX-射手沫蝴蝶",
    "contact": "QQ: 2094332348",
    "timestamp": 1718768000
}
```

---

## 🔒 安全声明

> ⚠️ **本程序仅供学习交流使用，严禁用于商业用途和非法目的。**
>
> 使用者应对其行为承担全部法律责任，开发者不承担任何直接或间接的责任。
>
> 如不同意本声明，请立即停止使用并删除本程序。

---

## 🛠️ 常见问题

### Q1: 所有接口都返回 404？

**A**：检查以下几点：
1. 服务器是否能访问外部网络（部分主机商禁用出站）
2. PHP `curl` 扩展是否启用：`php -m | grep curl`
3. 目标 API 线路是否仍有效（可手动测试单个接口）
4. `ZJK.txt` 中的自定义接口格式是否正确

### Q2: 响应很慢？

**A**：可以尝试：
1. 减少 API 线路数量（保留 5-8 条即可）
2. 在 `switch.php` 中调低 `global_api_timeout`
3. 启用缓存功能（修改 `index.php` 接入 `SmartCache`）
4. 启用代理池 + 合理设置速率限制，避免被接口限流

### Q3: 如何添加新的视频平台？

**A**：编辑 `config/platform.php`，新增一行：
```php
'平台标识' => '域名关键字|优先API名称',
```

### Q4: M3U8 链接没有被识别？

**A**：确认 `switch.php` 中 `enable_m3u8_direct = true`，URL 需满足：
- 路径包含 `.m3u8` 扩展名，或
- 查询参数包含 `m3u8` 关键字，或
- 路径中包含 `m3u8` 关键字

### Q5: 接口频繁被封禁？

**A**：在 `config/noad.php` 中启用代理池 + 设置合理的速率限制：
1. 设置 `enable_proxy = true`
2. 在 `proxies` 数组中填入可用代理（`ip:port`）
3. 设置 `rate_limit_min_interval_ms` 为 300-500 毫秒
4. 设置 `rate_limit_max_concurrent` 为 2-3

> 💡 **更多问题解答**：以上为精简 FAQ。关于 500 错误 / 404 接口失败 / 权限问题 / 升级版本 / 多份部署等完整解答，请访问
> **[📖 INSTALL.md · 完整常见问题](INSTALL.md#7%EF%B8%8F%E2%83%A3%E5%B8%B8%E8%A7%81%E9%97%AE%E9%A2%98-faq)**。

---

## 📝 更新日志

### v5.2.0 (2026-06-26) — 功能开关管理 + 解析结果集成去广告链接

- ✨ **后台系统设置新增功能开关分组**
  - 新增 `video_parse_enabled` 开关：视频解析接口总开关（控制 `/?url=`）
  - 新增 `ad_remove_url_enabled` 开关：去广告链接接口总开关（控制 `/q.php?url=`）
  - 新增 `parse_integrate_noad` 开关：解析结果集成去广告链接数组
  - 新增 `debug_mode_enabled` 开关：调试模式开关
  - 系统开关页面按功能分组展示：基础设置 / 去广告系统 / 功能开关

- ✨ **解析结果自动集成去广告链接数组**
  - 当视频解析 + 去广告 + 集成开关同时开启时，`/?url=` 返回结果中自动添加 `noad_urls` 数组
  - 包含多种去广告方式的链接：q.php 接口、短路径接口、NoAd 内置去广告、原始链接去广告
  - 每个链接包含类型、名称、URL、描述，方便前端按需选用
  - 同时支持 NoAd 模式和 Legacy 模式，M3U8 直链也支持
  - 自动识别当前协议和域名，生成正确的绝对路径链接

- ✨ **接口开关独立控制**
  - 视频解析接口（`/?url=`）和去广告接口（`/q.php?url=`）可独立启用/禁用
  - 关闭对应接口时返回友好的 503 提示，明确告知用户在后台开启
  - 开关粒度更细，方便不同场景下的功能管控

- 🔧 **配置新增**
  - `config/switch.php` 新增 4 个功能开关配置项
  - 配置即开即用，无需重启服务
  - 后台三个系统开关页面全部同步更新

### v5.1.2 (2026-06-26) — 系统开关配置面板 + /q 去广告专用接口

- ✨ **后台系统设置新增开关配置面板**
  - 新增 `noad_enabled` 开关：NoAd 去广告系统总开关
  - 新增 `sqlite_enabled` 开关：SQLite 数据库开关（指纹库持久化）
  - 新增 `ad_remove_enabled` 开关：去插播广告总开关（控制 /q 接口）
  - 在"系统开关"页面分组显示，一目了然

- ✨ **新增 `/q?url=` 去广告专用接口**
  - 简洁 JSON 格式返回，方便第三方调用
  - 自动调用 MD5 深度分析算法去除广告和插播
  - 生成本地缓存的纯净 M3U8 播放链接
  - 支持快速模式、强制重新分析、多进程控制
  - 支持广告特征码自动同步到指纹库
  - 智能缓存机制，30分钟内相同请求秒开
  - 受系统开关控制，未启用时返回明确提示

- 🔧 **配置新增**
  - `config/switch.php` 新增 3 个开关配置项（noad_enabled / sqlite_enabled / ad_remove_enabled）
  - 配置即开即用，无需重启服务

### v5.1.1 (2026-06-26) — .htaccess 伪静态支持 + 路径优化

- ✨ **新增 .htaccess 伪静态支持**
  - 支持直接访问 `域名/?url=xxx`，无需加 `/index.php?url=`
  - 自动 URL 重写，所有非文件/目录请求转发到 index.php
  - 新增安全防护：禁止直接访问数据库、缓存、日志文件
  - 新增静态资源缓存规则，提升访问速度

- 🔧 **优化 noad_md5.php 路径生成**
  - 修复反向代理 HTTPS 协议识别问题（支持 HTTP_X_FORWARDED_PROTO）
  - 优化路径计算逻辑，兼容根目录部署和子目录部署
  - 增强 SCRIPT_NAME 优先级，确保路径准确

### v5.1.0 (2026-06-26) — MD5深度分析全套 + NoAd专用API + 特征码自学习

- 🎯 **广告特征码自动同步机制（自学习算法）**
  - 深度分析识别的广告 MD5 自动存入指纹库（算法列表）
  - 指纹库越用越准，相同广告后续秒级识别
  - 支持白名单保护，正片内容永不误删
  - 新增 `syncAdSignaturesToDB()` 方法实现特征码同步
  - 新增 `getFingerprintStats()` 方法获取指纹库统计

- ✨ **noad_md5.php 对外 API 全面升级**
  - 专用去广告接口，用户直接调用获得无广告 M3U8
  - 自动后台运行多线程 MD5 深度分析
  - 自动过滤广告和插播片段
  - 自动同步广告特征码到指纹库
  - 生成 http/https 开头的纯净播放链接

- 🚀 **多进程 + curl_multi 双层并发架构**
  - 第一层：pcntl 多进程并行下载，自动检测 CPU 核心数（最高 16 进程）
  - 第二层：每个子进程内使用 curl_multi 并发下载多个 TS 片段
  - 比单进程串行模式提升 5-10 倍下载速度
  - 比单纯多进程模式再提升 2-3 倍

- ✨ **MD5 深度分析增强**
  - 新增 Master Playlist 自动递归解析，支持多分辨率 m3u8 索引文件
  - 新增 DISCONTINUITY 插播点识别，区分广告插播和场景切换
  - 新增广告集群合并分析，识别连续广告片段
  - 新增 TS 特征码提取（MD5 指纹、文件名模式、大小分布）
  - 新增后台广告插播合并分析面板和 TS 特征码展示面板
  - 新增特征码导出功能，支持 JSON 格式下载
  - 启发式预检测：URL 强特征 + 时长严重异常的片段免下载直接判定
  - 智能抽样模式：片头 + 片尾 + 中间均匀抽样，仅分析约 25% 片段

- 🛠️ **沫兮客户端自动更新脚本 `update.sh`**
  - 自动从 GitHub 拉取最新代码并解压更新
  - 自动备份当前版本（支持 tar.gz 压缩）
  - 一键回滚到上一版本
  - 交互式数字菜单，输入数字选择功能
  - 支持国内镜像加速（ghproxy）

- ⚡ **缓存与性能优化**
  - 智能缓存：30 分钟内相同 URL 秒开播放
  - 异步模式 `async=1`：秒开 + 后台静默更新
  - 缓存文件锁：防止并发重复分析
  - 新增 `sync=1/0` 参数控制特征码自动同步
  - 返回数据增强：sync_result、fingerprint_stats、cached_at 等

- 🔧 **配置新增**
  - `md5_auto_sync_signatures`：自动同步广告特征码（默认 true）
  - `md5_num_processes`：默认多进程数（默认 8）
  - `md5_max_concurrency`：默认并发数（默认 12）
  - `cache_ttl_seconds`：缓存有效期（默认 1800 秒）

### v5.0.0 (2026-06-25) — 后台架构大重构 · 5 大模块分类 · 功能更清晰

- ✨ **后台架构大重构（7 → 5 大模块）**：重新梳理功能分类，将原先分散的 7 个主标签 + 子标签结构，重新组织为 **5 大核心模块**，逻辑更清晰，操作更直观
  - 📊 **仪表盘**：系统运行状态总览、快速开始、环境检查、操作日志
  - 🔧 **解析配置**：整合解析接口管理、自定义解析、平台规则、API 设置、调用日志、解析日志、接口测试，一站式管理所有解析相关配置
  - 🚫 **去广告配置**：整合 MD5 深度分析、总览、资源站设置、广告关键词规则、解析源管理，专注于去广告功能的配置与优化
  - 🧩 **算法**：整合算法列表、如意算法、MD5 指纹算法、万能规则 2、M3U8 解析测试，所有算法相关配置集中管理
  - ⚙️ **系统设置**：整合配置备份、修改密码、后台设置、缓存管理、工具管理，系统级配置统一入口
- ✨ **解析配置模块独立化**：将第一个核心功能（用户后台配置解析源）独立为完整模块，包含解析接口管理、自定义解析、平台规则、API 设置、调用日志、解析日志、接口测试 7 个子功能
- ✨ **去广告配置模块独立化**：将第二个核心功能（MD5 分析去广告）独立为完整模块，包含 MD5 深度分析、总览、资源站设置、广告关键词规则、解析源管理 5 个子功能
- ✨ **算法模块集中管理**：所有算法（如意算法、MD5 指纹算法、万能规则 2、自定义算法）统一在「算法」模块中管理，方便调试和优化
- ✨ **系统设置模块统一入口**：配置备份、密码修改、后台安全设置、缓存管理、工具管理全部集中在「系统设置」模块
- ✅ **所有原有功能完整保留**：重构仅调整导航和页面组织方式，所有 PHP 业务逻辑、AJAX 接口、表单处理、数据库操作均保持不变
- ✅ **向后兼容性保障**：旧的标签面板 DOM 结构保留（display:none），确保所有 JavaScript 功能正常运行
- ✅ **冒烟测试全通过**：16/16 项冒烟测试全部通过，核心功能零回归

### v4.4.0 (2026-06-22) — 后台导航精简 + 智能自动保存 + 功能整合

- ✨ **导航大精简（19 → 7 标签）**：将原先分散的 19 个独立 Tab 重新组织为 **7 个主标签 + 多层子标签** 结构，逻辑更清晰，操作更流畅
  - 📼 **智能解析中心**：合并 M3U8 解析 / 资源站点 / 解析日志 / 缓存管理 / 解析源
  - 🚫 **智能去广告**：合并 NoAd 数据统计 / 广告规则库 / NoAd 设置 / 自定义算法，新增如意算法参数独立配置
  - 📡 **线路配置**：合并 API 线路配置 / 平台规则配置
  - 🔌 **接口中心**：合并系统开关 / 自定义接口 / 接口测试
  - 🛠️ **系统管理**：合并配置备份 / 修改密码 / 后台设置
- ✨ **智能自动保存功能**：点击页面空白区域时，自动将当前子标签页表单保存到浏览器 localStorage；3 秒冷却避免频繁触发；显示浮动保存成功提示
- ✨ **一键还原功能**：在「智能去广告 → 总览与设置」「系统管理 → 后台设置」等关键页面，新增「↩ 还原上次自动保存」按钮，避免误操作损失配置
- ✨ **子标签页切换系统**：主标签内的子功能通过二级导航切换，渐变背景视觉分层，滚动条平滑，操作体验丝滑
- ✨ **如意算法参数独立配置页**：基准时长 / 容差 / 短片段阈值 / 簇长度 / Score 阈值 / DISCONTINUITY 信号 / 调试模式 独立控制

### v4.3.0 (2026-06-21) — 单文件后台整合 + 代理/速率限制反封禁

- ✨ **后台文件整合（3 → 1）**：将原先分散的 `admin.php`（入口逻辑）、`admin_tpl.php`（模板）、`admin_main.php`（旧模板）、`admin_style.css`（样式）**整合为单个 `admin.php`**
  - CSS 样式代码内联渲染，无需额外请求样式文件
  - 18 个功能 Tab 的 HTML 模板通过 PHP 函数动态生成
  - 登录校验、权限控制、表单处理、AJAX 响应全部统一管理
  - 部署更简单（只需上传一个文件），版本升级更方便
- ✨ **代理与反封禁**：新增 `config/noad.php` 中的代理池轮换配置、请求速率限制（最小间隔 + 随机抖动）、最大并发数控制、随机 User-Agent 池
- ✨ **admin.php 语法检查通过**：`php -l admin.php` 零错误
- ✨ **冒烟测试通过**：`tests_smoke.php` 16/16 全部通过（入口响应 / AJAX 接口 / 算法扫描 / M3U8 解析）
- 📖 **README 与项目结构说明** 更新到 v4.3.0，新增整体架构图

### v4.2.0 (2026-06-21) — 自定义算法扩展 + 项目结构整理

- ✨ **`algorithms/` 目录体系**：新增 `AbstractAlgorithm` 基类 + `AlgorithmRegistry` 注册表，所有算法自动扫描加载，按优先级排序，按作用域应用
- ✨ **5 个内置示例算法**：URL 跟踪参数清洗、广告关键词过滤、域名重写、M3U8 片段级清洗、协议规范化
- ✨ **用户自定义算法模板**：`algorithms/custom_template.php` 可复制、改名，快速编写专属过滤规则
- ✨ **`noad_proxy.php` 新增 `mode=algorithms`**：提供 `list` / `toggle` / `reload` / `test` 四个 JSON API，便于后台管理或被第三方系统调用
- ✨ **后台新增「🧩 自定义算法」Tab**：列表展示算法信息 / 一键启用禁用 / 重新扫描目录 / 本地测试算法链 / 附开发示例代码
- ✨ **`NoAdParser::listCustomAlgorithms()` / `setCustomAlgorithmEnabled()` / `applyCustomAlgorithms()` / `reloadCustomAlgorithms()`**：供 PHP 内部或 AJAX 直接调用
- ⚙️ **`config/noad.php` 新增 `enable_custom_algorithms` + `custom_algorithms_scope`**
- 🧹 **项目结构整理**：剔除历史版本 `.zip`、旧版备份目录、临时 `test_playlist.m3u8`、`cache/*.db` 等大体积/运行时文件；完善 `.gitignore`，仅保留 `backup/.gitkeep` 与 `cache/.gitkeep`

### v4.1.0 (2026-06-20) — M3U8 解析页 + 资源站去广告

### v4.0.0 (2026-06-19) — NoAd 去广告解析系统

- ✨ **M3U8 广告片段智能过滤**：自动识别 `#EXTINF` 片段中的广告关键词
- ✨ **7 种资源类型分类管理**：电影 / 电视剧 / 综艺 / 动漫 / 纪录片 / 体育 / 短视频
- ✨ **多源自动匹配**：并发请求多个解析源，自动选择最快有效响应
- ✨ **SQLite 轻量数据统计**：自动记录每次解析请求（时间 / IP / 来源 / 耗时 / 移除广告片段数）
- ✨ **缓存加速**：30 分钟内相同 URL 请求直接返回缓存结果
- ✨ **代理入口 `noad_proxy.php`**：支持 `mode=m3u8`（代理并清洗远程 M3U8）、`mode=ts`（TS 片段跨域代理）、`mode=api`（直接 JSON 响应）
- ✨ **全新后台管理可视化**：从 v3.1 升级到 12+ 个 Tab

---

## 👤 开发者信息

| 项目 | 内容 |
|------|------|
| **开发者** | MX-射手沫蝴蝶 |
| **联系方式** | QQ: 2094332348 |
| **当前版本** | v5.2.0（功能开关管理 + 解析结果集成去广告链接） |
| **更新日期** | 2026-06-26 |

---

<div align="center">

**Made with ❤️ by MX-射手沫蝴蝶**

</div>
