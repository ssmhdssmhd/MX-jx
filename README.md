...
**PHP 智能线路切换系统 · NoAd 去广告 · 单文件后台 v5.0.0**
...
- ✨ **新增 `noad_md5.php` 去插播专用接口**
  - 自动运行 MD5 深度分析，智能识别并去除广告片段
  - 支持 `mode=redirect`（默认，302 跳转）和 `mode=json`（完整 JSON 结果）
  - 支持 `fast=1` 快速模式（智能抽样约 25% 片段，速度提升 4 倍）
  - 支持 `force=1` 强制重新分析（忽略缓存）
  - 自动缓存分析结果，默认缓存 30 分钟
  - 返回广告插播段、广告特征码、MD5 指纹等详细数据
  - 生成的去广告链接以 http:// 或 https:// 开头，可直接使用

### v4.5.4 (2026-06-25) — update.sh 交互式数字菜单
...
### v4.5.1 (2026-06-25) — MD5 深度分析速度大幅提升
